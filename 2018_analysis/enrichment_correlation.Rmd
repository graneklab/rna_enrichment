...
```{r}
genecounts_lanes_combined %>%
  dplyr::select(-c("enrichment_method", "RNA_sample_num")) %>%
  column_to_rownames(var="library") %>%
  as.matrix %>%
  t ->
  genecounts_for_desesq
head(genecounts_for_desesq)
```

```{r}
genecounts_lanes_combined %>%
  dplyr::select(library, enrichment_method, RNA_sample_num) %>%
...
dds <- DESeqDataSetFromMatrix(countData = genecounts_for_desesq,
                              colData = sample_df,
