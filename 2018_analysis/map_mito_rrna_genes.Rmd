...
```{r}
tot_bam_files = dir_ls(total_samples_starout.dir,glob = "*TOT*L00*.bam") 
tot_bams = BamFileList(tot_bam_files, paste0(tot_bam_files, ".bai"))
tot_bam_files
```

```{r}
tot_L1_bam_files = dir_ls(total_samples_starout.dir,glob = "*TOT*L001*.bam") 
tot_L1_bams = BamFileList(tot_L1_bam_files, paste0(tot_L1_bam_files, ".bai"))
tot_L1_bam_files
```
# Whole Genome
```{r}
autoplot(tot_L1_bams, which = whole_mito_gr) +
  xlim(whole_mito_gr) +
...
```{bash}
grep CNAG_10500 $TOTAL_SAMPLES_STAROUT/*_L001_ReadsPerGene.out.tab
```

# Figures for Manuscript
## Whole Genome
```{r}
autoplot(tot_L1_bams[[1]], which = whole_mito_gr) +
  xlim(whole_mito_gr) +
  xlab("Position on Mitochondrial Chromosome") +
  ylab("Read Coverage") +
...
ggsave(file.path(fig.dir, "whole_mito_rrna_coverage.tiff"), plot=whole_mito_plot@ggplot, compression = "lzw")
```
