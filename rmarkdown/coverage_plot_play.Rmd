...
df <- data.frame(chr="chr1", start=11:15, end=12:16,
                 strand=c("+","-","+","*","."), score=".", feature="exon", source=)
df
...
as.data.frame(gr) %>%
  select(seqname", "source", "feature", "start", "end", "score", 
                       "strand", "frame", "attributes")
```

```{r}
GRangesList(gene_15s, gene_21s) %>%
  as.data.frame()
```
```{r}
# as.data.frame(gene_15s)
