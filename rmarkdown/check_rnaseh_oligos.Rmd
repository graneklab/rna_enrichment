...
reads that map to 
```{bash}
head ${MITO_RRNA_STAR_OUT}/*.out
ls ${MITO_RRNA_STAR_OUT}/

head ${MITO_RRNA_STAR_OUT}/rrna_oligos_ReadsPerGene.out.tab

```
```{bash}
sort -nr -k4 ${MITO_RRNA_STAR_OUT}/rrna_oligos_ReadsPerGene.out.tab | head
```
...
# 
# generated_oligo_counts %>%
#   summarise_at(total_counts)
```

```{bash}
head /workspace/2018_tot_samples/seq_out/*
```
