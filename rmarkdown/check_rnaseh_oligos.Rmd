...
```{bash}
samtools view ${MITO_RRNA_STAR_OUT}/rrna_oligos_Aligned.sortedByCoord.out.bam | grep -v NH:i:1 | cut -f1,3-4
```
...
```{r}
countBam(oligo_bam.file,
         param=ScanBamParam(which=h99_granges)) %>%
  filter(records > 0) %>%
  dplyr::select(chr=space, start, end) %>%
...
```{r}
o = findOverlaps(h99_granges, rrna_homologous.gr)
grl1 = split(h99_granges[queryHits(o)], 1:length(o)) # You can't mendoapply on a GRanges object
grl2 = split(rrna_homologous.gr[subjectHits(o)], 1:length(o))
foo = function(x, y) {
  rv = x
