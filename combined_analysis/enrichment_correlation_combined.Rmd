...
res50 <- results(dds50, contrast = c("enrichment_method", "Poly(A)", "Unenriched"), lfcThreshold=lfc_cutoff, altHypothesis = "less")
res50

res50 %>% 
  as.data.frame() %>%
  arrange(padj, log2FoldChange) -> res50_df

rownames_to_column(res50_df, "gene_id") -> res50_df_geneids

res50_df_geneids %>% 
  filter(padj < sig_cutoff) %>%
  pull(gene_id) ->
  sig_less_polya_genes

norm_counts_long_noreps_missed_polya <- norm_counts_long_noreps %>%
  mutate(polya_missed = ifelse(geneid %in% sig_less_polya_genes, T, F))

norm_counts_long_noreps_missed_polya
```
...
norm_counts_long_noreps_missed_polya %>%
    ggplot(aes(x=gold_mean, y=norm_count, colour=polya_missed)) +
    geom_point(alpha=0.3, colour = norm_counts_long_noreps_missed_polya$polya_missed) +
    labs(x = "Average Normalized Count in Unenriched", y= "Normalized Count") +
    theme_bw() +
    theme(text=element_text(size=10)) +
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())  +
