...
unenriched_cor = calculateCrossCorrelation(unenriched_norm_counts, dds)
```
...
plotCorrelationValues(unenriched_cor, pearson_df, dds, fig_palette, enrich_levels)
```
...
correlationRanges(unenriched_cor, pearson_df) %>%
  kable
```
