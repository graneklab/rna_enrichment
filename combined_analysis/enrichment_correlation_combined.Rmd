...
ggsave(file.path(fig.dir, "all_genes_cor_values.tiff"))
```

```{r fig.width=8, fig.height=8}  
makeEnrichmentScatterplot(norm_counts_long_noreps, fig_palette, enrich_levels)
ggsave(file.path(fig.dir, "all_genes_scatter.tiff"))
```
...
ggsave(file.path(fig.dir, "protein_coding_cor_values.tiff"))
```
...
ggsave(file.path(fig.dir, "protein_coding_scatter.tiff"))
```
...
plotCorrelationValues(ncrna_unenriched_cor, 
                      ncrna_enriched_cor, 
                      dds, fig_palette, enrich_levels)
ggsave(file.path(fig.dir, "ncrna_cor_values.tiff"))
```
...
norm_counts_long_noreps %>%
  filter(geneid %in% ncrna_geneids) %>%
  makeEnrichmentScatterplot(fig_palette, enrich_levels)
ggsave(file.path(fig.dir, "ncrna_scatter.tiff"))
```
