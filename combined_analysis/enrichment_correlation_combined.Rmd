...
norm_counts_long %>%
  ...
  filter(!(enrichment_method %in% c("PolyA","Total") &
             exp_year == 2018)) %>%
...
```{r}
norm_counts_long %>%
  # only plot one tech rep per library: remove RNAseH rep2 and PolyA and Total from 2018
  filter(enrichment_rep != 2) %>%
  filter(!(enrichment_method %in% c("PolyA","Total") &
             exp_year == 2018)) %>%
  ggplot(aes(x=tot_mean, y=norm_count, color=enrichment_method)) +
  geom_point(alpha=0.3) +
  labs(x = "Average Normalized Count in Total", y= "Normalized Count") +
  theme_bw() +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) ->
...
all_library_scatterplot +
  ...
  scale_colour_brewer(palette = fig_palette, guide = "legend") +
  geom_abline()
```
