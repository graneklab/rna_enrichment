...
dea_polya_unenriched %>% 
  as.data.frame() %>%
  arrange(padj, log2FoldChange) -> dea_polya_unenriched_df

rownames_to_column(dea_polya_unenriched_df, "gene_id") -> dea_polya_unenriched_df_geneids

dea_polya_unenriched_df_geneids %>% 
  filter(padj < sig_cutoff) %>%
  pull(gene_id) ->
  lost_polya_genes

norm_counts_long_noreps_lost_polya <- norm_counts_long_noreps %>%
  mutate(polya_lost = ifelse(geneid %in% lost_polya_genes, T, F))
...
norm_counts_long_noreps_lost_polya$polya_lost <-  as.factor(norm_counts_long_noreps_lost_polya$polya_lost)
...
norm_counts_long_noreps_lost_polya %>%
    ...
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())  +
    facet_grid(enrichment_method ~ RNA_sample_num) +
    scale_colour_brewer(palette = fig_palette2, direction = -1, guide = FALSE, drop=TRUE, limits = sig_levels) +
    geom_abline(size=0.5) %>%
    return

ggsave(file.path(fig.dir, "polya_dep_S6.tiff"), compression = "lzw", width=20, height=20, units = "cm")
```
```{r}
import(gtf_with_mito_rrna.file, format = "gtf") %>%
  as.data.frame %>%
  filter(gene_id %in% lost_polya_genes) %>%
  select(gene_id, seqnames, gene_biotype) -> genes_lost_in_polya
```
